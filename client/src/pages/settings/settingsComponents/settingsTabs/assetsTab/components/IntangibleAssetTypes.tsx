...
import { Plus, Edit, Trash2, Layers, AlertTriangle, Search, Download, Upload } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function IntangibleAssetTypes({
  ...
  const handleExportCSV = () => {
    ...
    exportToCSV('intangible-types', exportData);
  };
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel('intangible-types', exportData);
  };

  const handleImport = async (rows: Record<string, string>[]): Promise<ImportResult> => {
    const resolvedRows = await resolveImportIds('intangible-types', rows);
    const existingNames = types.map(t => t.name);
    const { uniqueRows } = checkDuplicates('intangible-types', resolvedRows, existingNames);
    const result = await importRows('intangible-types', uniqueRows);
    if (result.created > 0) {
      fetchTypes();
