...
} = require('../../utils/transferFormNumber.js');

describe('transferFormNumber', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  describe('generateTransferFormNumberFallback', () => {
    it('starts the sequence at 0001 with MMYYYY date when no forms exist', async () => {
      ...
      const result = await generateTransferFormNumberFallback();
      expect(result).toBe(`TRF-${dateStr}-0001`);
      expect(mockPool.execute).toHaveBeenCalledWith(
        expect.stringContaining('FROM asset_transfer_forms'),
        [`TRF-%${now.getFullYear()}-%`]
      );
    });
    ...
    it('continues the sequence across months within the same year', async () => {
      ...
      const dateStr = `${String(now.getMonth() + 1).padStart(2, '0')}${now.getFullYear()}`;
      const result = await generateTransferFormNumberFallback();
      expect(result).toBe(`TRF-${dateStr}-0016`);
    });

    it('resets to 0001 when only prior-year forms exist (year-scoped lookup)', async () => {
      mockPool.execute.mockResolvedValue([[{ next_seq: 1 }], []]);
      const result = await generateTransferFormNumberFallback();
      expect(result).toMatch(/^TRF-\d{6}-0001$/);
    });
  });

  describe('generateTransferFormNumber', () => {
    it('falls back when no settings exist', async () => {
      mockPool.execute.mockImplementation(async (sql: string) => {
        if (sql.includes('asset_transfer_form_settings')) return [[], []];
        return [[{ next_seq: 3 }], []];
      });
      const now = new Date();
      const dateStr = `${String(now.getMonth() + 1).padStart(2, '0')}${now.getFullYear()}`;
      const result = await generateTransferFormNumber('c1', null);
      expect(result).toBe(`TRF-${dateStr}-0003`);
    });

    it('scopes the sequence per year for MMYYYY date format', async () => {
      mockPool.execute.mockImplementation(async (sql: string) => {
        if (sql.includes('asset_transfer_form_settings')) {
          return [[{ company_format: 'code', department_format: 'none', include_date: 1, date_format: 'MMYYYY', it_asset_transfer_code: '1009', admin_asset_transfer_code: '1009' }], []];
        }
    ...
    it('scopes the sequence per year even for a non-standard month-inclusive date format', async () => {
      mockPool.execute.mockImplementation(async (sql: string) => {
        if (sql.includes('asset_transfer_form_settings')) {
          return [[{ company_format: 'code', department_format: 'none', include_date: 1, date_format: 'MM-DD-YYYY', it_asset_transfer_code: '1009', admin_asset_transfer_code: '1009' }], []];
        }
        if (sql.includes('FROM companies')) return [[{ code: '005' }], []];
        return [[{ next_seq: 2 }], []];
      });
    ...
    it('scopes the sequence per day for YYYYMMDD date format', async () => {
      mockPool.execute.mockImplementation(async (sql: string) => {
        if (sql.includes('asset_transfer_form_settings')) {
          return [[{ company_format: 'code', department_format: 'none', include_date: 1, date_format: 'YYYYMMDD', it_asset_transfer_code: '1009', admin_asset_transfer_code: '1009' }], []];
        }
        if (sql.includes('FROM companies')) return [[{ code: '005' }], []];
        return [[{ next_seq: 4 }], []];
      });
