-- Include the assignee's ACTIVE accountability form in each intangible asset's
-- form list, even when the form snapshot (`assets_data`) predates the intangible
-- assignment. Mirrors the server-side `findFormsByAssetId` fix:
--
--   af.asset_id = ia.id
--   OR af.assets_data LIKE '%ia.id%'
--   OR af.formID IN (
--        active forms of users who currently hold an Active assignment of ia.id
  ...
  SELECT
    ...
    (
      ...
      WHERE af.`deleted_at` IS NULL
        AND (
          af.`asset_id` = ia.`id`
          OR af.`assets_data` LIKE CONCAT('%', ia.`id`, '%')
          OR af.`formID` IN (
            SELECT af2.`formID`
