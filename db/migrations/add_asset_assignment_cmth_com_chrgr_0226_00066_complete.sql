...
-- Updates asset status to 'Assigned' and sets department/location fields
-- Updates accountability form assignment_id to link to the new assignment
...
-- Get asset's current location values (users table doesn't have location fields)
SELECT @location_id := location_id FROM assets WHERE assetID = @asset_id;
SELECT @location_room_id := location_room_id FROM assets WHERE assetID = @asset_id;
...
-- Update the asset's status to 'Assigned' and set department/location fields
UPDATE assets
SET 
  status = 'Assigned',
...
SELECT CONCAT('Asset: ', @asset_code, ' assigned to User: ', @user_employee_number) AS assignment_details;
SELECT CONCAT('Asset status updated to: Assigned') AS asset_status_updated;
SELECT CONCAT('Accountability form assignment_id updated') AS form_assignment_updated;
SELECT CONCAT('Linked to Accountability Form: ', @form_number) AS form_linked;

-- Display final state for verification
SELECT assetID, asset_code, name, status, department_id, location_id, location_room_id FROM assets WHERE assetID = @asset_id;
SELECT assignmentID, asset_id, user_id, department_id, status FROM asset_assignments WHERE assignmentID = @assignment_id;
SELECT formID, form_number, assignment_id, user_id FROM accountability_forms WHERE formID = @form_id;
