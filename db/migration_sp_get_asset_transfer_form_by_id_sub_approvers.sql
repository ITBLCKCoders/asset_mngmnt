-- Migration: Update sp_get_asset_transfer_form_by_id to return sub_approver_1 and sub_approver_2 columns.
-- Run after migration_add_sub_approver_signatures_all_forms.sql so asset_transfer_forms has those columns.
...
BEGIN
  SELECT
    ...
    atf.sub_approver_1_digital_signature,
    atf.sub_approver_1_signed_by,
    DATE_FORMAT(atf.sub_approver_2_signed_at, '%Y-%m-%d %H:%i:%s') AS sub_approver_2_signed_at,
    atf.sub_approver_2_digital_signature,
    atf.sub_approver_2_signed_by
...
-- End of migration_sp_get_asset_transfer_form_by_id_sub_approvers.sql