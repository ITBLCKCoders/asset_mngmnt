-- Migration: Add sub_approver_1 param to custodian/approver stored procedures.
-- Run after migration_add_sub_approver_1_and_company_approvers.sql.
-- The sub_approver_1 column already exists in user_custodian_settings and
-- asset_mngmnt_roles; this recreates the procedures so controllers can persist it.

-- 0. sp_get_users with sub_approver_1 so the Assign Role tab reflects the
--    stored per-user custodian flags (recreates the same procedure shipped in
--    migration_add_sub_approver_1_and_company_approvers.sql for databases that
--    only applied the ALTER TABLEs).
...
-- 1. sp_upsert_user_custodian_settings with sub_approver_1 (11 params)
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_upsert_user_custodian_settings`(
    ...
    IN p_sub_approver_1 TINYINT
)
BEGIN
    INSERT INTO user_custodian_settings (
        user_id, access_add_edit, access_assignment, access_return,
        hr_accountability_receiver, manager_approver_1, manager_approver_2, manager_approver_3, finance_approver, sub_approver_2, sub_approver_1
    ) VALUES (
        p_user_id, p_access_add_edit, p_access_assignment, p_access_return,
        p_hr_accountability_receiver, p_manager_approver_1, p_manager_approver_2, p_manager_approver_3, p_finance_approver, p_sub_approver_2, p_sub_approver_1
    )
    ON DUPLICATE KEY UPDATE
        ...
        sub_approver_1 = p_sub_approver_1,
        updated_at = CURRENT_TIMESTAMP;
END ;;
DELIMITER ;

-- 2. sp_get_user_custodian_settings with sub_approver_1
...
-- 3. sp_create_role with sub_approver_1 (15 params)
...
-- 4. sp_update_role with sub_approver_1 (16 params)
