-- Migration: add finance_approver column to user_custodian_settings and asset_mngmnt_roles
-- Run this against your MySQL database.

-- 1. Add finance_approver to user_custodian_settings
ALTER TABLE user_custodian_settings
  ADD COLUMN `finance_approver` TINYINT NOT NULL DEFAULT 0 AFTER manager_approver_3;

-- 2. Add finance_approver to asset_mngmnt_roles
ALTER TABLE asset_mngmnt_roles
  ADD COLUMN `finance_approver` TINYINT NOT NULL DEFAULT 0 AFTER manager_approver_3;

-- 3. Drop and recreate sp_upsert_user_custodian_settings with finance_approver param
...
-- 4. Drop and recreate sp_get_user_custodian_settings with finance_approver
...
-- 5. Drop and recreate sp_get_users with finance_approver
