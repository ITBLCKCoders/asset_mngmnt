...
BEGIN
    ...
    INSERT INTO asset_categories (
        categoryID, name, prefix, gl_code, company_id, created_by, updated_by
    ) VALUES (
        new_id, p_name, p_prefix, p_gl_code, p_company_id, p_created_by, p_created_by
    );
...
BEGIN
  INSERT INTO asset_mngmnt_roles (name, description, created_by, updated_by)
  VALUES (p_name, p_description, p_created_by, p_created_by);
...
BEGIN
  SELECT * FROM asset_categories WHERE deleted_at IS NULL AND company_id = p_company_id ORDER BY created_at DESC;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN
  UPDATE asset_mngmnt_roles
  SET name = p_name, description = p_description, updated_by = p_updated_by, updated_at = CURRENT_TIMESTAMP
  WHERE roleID = p_roleID AND deleted_at IS NULL;
  SELECT ROW_COUNT() as affected_rows;
END ;;
DELIMITER ;
...
-- Dump completed on 2025-12-19 16:03:03
